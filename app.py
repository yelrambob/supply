...
# Pull GitHub copies of order_log.csv and last_order.csv at startup
...
def read_emails() -> pd.DataFrame:
    ...
    if "email" in df.columns:
        ...
        for _, r in df.iterrows():
            email = extract_email(r.get(first_col, ""))
            if email:
...
st.caption(
    ...
    f"Email configured: {'✅' if smtp_ok() else '❌'} • Recipients: {len(all_recipients(emails_df))}"
)

# (UI code unchanged from your version – omitted here for brevity)
# Keep your "Create Order", "Adjust Inventory", "Catalog", "Order Logs", and "Tools" tabs exactly as before.
